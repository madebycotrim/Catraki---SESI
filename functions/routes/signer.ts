...
signerRouter.post('/sign', rateLimiter({ limit: 10, windowSeconds: 60, keyPrefix: 'sign_doc' }), async (c) => {
  ...
  const manifestData = {
    ...
    termos_versao: parsed.data.termos_versao || '1.0.2026',
  };
  ...
  const pdfBytes = await GeradorPdfTermoSesi.gerarPdfOriginal({
    ...
    ipAddress: ipAddress,
    ipVersion: cfData.ipVersion,
    userAgent: userAgent,
    geoCidade: geoCity,
    geoEstado: geoRegion,
    geoPais: geoCountry,
    geoLocalizacaoCompleta: cfData.formattedLocation,
    provedorAsn: cfData.asnOrg ? `${cfData.asnOrg} (AS${cfData.asnNumber})` : undefined,
  ...
  try {
    ...
    const batch = await db.batch([
      ...
      db.prepare(
        ...
        parsed.data.auth_health || 'yes',
        parsed.data.auth_data || 'yes',
        parsed.data.auth_image || 'no',
        pdfR2Key,
        signedAtIso,
        docParentHash,
        doc.id
