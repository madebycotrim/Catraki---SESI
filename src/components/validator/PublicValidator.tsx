    ...
    <div className="animate-in fade-in duration-500 max-w-4xl mx-auto px-2 sm:px-4 pb-12 pt-2">
      ...
      >
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <h2 style={{ fontSize: '10.5pt', fontWeight: 'bold', color: '#334155', margin: 0 }}>
          ...
          {!validationResult && (
            <div className="space-y-8">
              <p style={{ textAlign: 'justify', textIndent: '1.25cm', color: '#1e293b', margin: 0, lineHeight: '1.7', fontSize: '11pt' }}>
              ...
              <form
                ...
                className="space-y-5 bg-slate-50/80 border-2 border-slate-200 rounded-xl p-6 sm:p-7 shadow-xs"
              >
                <div className="space-y-2">
                  <label className="text-xs sm:text-sm font-bold text-slate-900 uppercase tracking-wide flex items-center gap-2">
                    ...
                    Código Único de Validação ou Hash SHA-256
                  </label>
                  <div className="relative">
                    <input
                      ...
                      autoFocus
                      className="w-full px-4 py-3 bg-white border border-slate-300 rounded-lg font-mono text-sm sm:text-base uppercase text-slate-900 tracking-wider focus:outline-none focus:border-sesi-primary focus:ring-2 focus:ring-sesi-primary/20 transition-all shadow-xs"
                    />
                  </div>
                ...
                {errorMessage && (
                  <div className="p-3.5 bg-red-50 border border-red-200 rounded-lg text-red-700 text-xs sm:text-sm flex items-center gap-2.5">
                ...
                <button
                  ...
                  className="w-full py-3 px-6 bg-sesi-primary hover:bg-blue-900 text-white font-bold text-xs sm:text-sm rounded-lg flex items-center justify-center gap-2 transition-colors shadow-sm disabled:bg-slate-300 disabled:text-slate-500 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      Consultando Registro Criptográfico...
                    </>
                  ) : (
                    <>
                      <Search className="w-4 h-4" />
                      Consultar e Validar Documento
                    </>
                  )}
                </button>
              </form>
              ...
              <div className="space-y-2.5 pt-2 text-xs text-slate-600 border-t border-slate-200">
                <div className="flex items-center gap-2 font-bold text-slate-800 text-xs sm:text-sm">
                  ...
                  Informações sobre a Trilha de Auditoria e Integridade
                </div>
                <p className="leading-relaxed m-0 text-justify text-xs" style={{ textIndent: '1.25cm' }}>
            ...
            <div className="space-y-6">
              ...
              <div className="text-center pb-4 border-b border-slate-200 space-y-1.5">
                <div className={`inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${
                  ...
                  {validationResult.document_status === 'signed' ? 'Assinatura Verificada' : 'Status: ' + validationResult.document_status}
                </div>
                
                <h3 className="text-base font-bold text-slate-900 uppercase tracking-wide m-0">
              ...
              <div className="bg-blue-50/80 border-2 border-blue-200 rounded-xl p-4 sm:p-5 flex items-center justify-between">
                <div>
                  <span className="block text-xs font-bold text-blue-900 uppercase tracking-wider">
                    Código Único de Validação
                  </span>
                  <strong className="text-lg sm:text-xl font-mono font-bold text-sesi-primary tracking-wider">
                ...
                <span className="text-xs sm:text-sm font-bold text-emerald-800 bg-emerald-100 border border-emerald-200 px-3 py-1.5 rounded-md">
                  Autenticado ✓
                </span>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-4">
                <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wide flex items-center gap-2 m-0 border-b border-slate-200 pb-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
                  <div>
                    ...
                    <strong className="text-slate-800 font-semibold text-sm">{validationResult.procedure_title}</strong>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Estudante Beneficiado</span>
                    <strong className="text-slate-800 font-semibold text-sm">{validationResult.minor_name_initials}</strong>
                  </div>
                  <div className="sm:col-span-2">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Responsável Legal (Assinante)</span>
                    <strong className="text-slate-800 font-semibold text-sm">{validationResult.signer_name} (CPF: {validationResult.signer_cpf_masked})</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wide m-0 border-b border-slate-200 pb-2">
                  2. Evidências Digitais de Validação
                </h4>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="sm:col-span-2 bg-white border border-slate-200 rounded-lg p-3.5">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Código Criptográfico de Integridade (Hash)</span>
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-100 font-mono text-xs text-slate-700 break-all select-all">
                  ...
                  <div className="sm:col-span-2 bg-white border border-slate-200 rounded-lg p-3.5 space-y-1.5">
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-100 font-mono text-[10px] text-slate-500 break-all">
